 ...
 *
 * The character encoding used in this file is UTF-8. Some comments include 
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
{
    ...
    destV->a[0] = (srcV1->a[1] * srcV2->a[2]) - (srcV1->a[2] * srcV2->a[1]);
    destV->a[1] = (srcV1->a[2] * srcV2->a[0]) - (srcV1->a[0] * srcV2->a[2]);
    destV->a[2] = (srcV1->a[0] * srcV2->a[1]) - (srcV1->a[1] * srcV2->a[0]);
    return destV;
}
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
{
    ...
    for (i = 0; i < 3; i++) {
        for (j = 0; j < 3; j++) {
            destM->a[i][j] = 0.0;
            for (k = 0; k < 3; k++) {
                destM->a[i][j] += srcM1->a[i][k] * srcM2->a[k][j];
            }
        }
    }
    return destM;
...
/*! \page page-why-struct-array Why define vector and matrix arrays as structs?
    ...
    This time, you can't even compile the code, and the compiler tells you what
    is wrong. With the clang compiler, the messages are as follows:

        vec2.c:12:21: error: passing 'V3D_Vector' to parameter of incompatible type
              'double *'
        v3d_rectToPolar(vec, &azimuth, &elevation);
                        ^~~
